...
namespace ak { namespace gen {
   ...
   void HelpOp::execute( ParseOptionsBase const & pob ) {
      LOG_CONSOLE("");
      LOG_CONSOLE("   usage  ak_generate -s PATH_1 [ -s PATH_2 -s PATH_3 ... ] [ --verbose ] [ --dry-run ] [--media=audio/art/lyrics/video]");
      LOG_CONSOLE("");
      LOG_CONSOLE("   # example 1: Copyng from different sources in verbose mode\n")
      LOG_CONSOLE("   ak_generate -s /Vol/media/a -s /Vol/media/d --verbose [--media=audio] ")
      LOG_CONSOLE("\n");
